  ...
  var Node = function (obj, parent, label)
  {
    ...
    this.el.mouseup(function () {
      ...
      window.setTimeout(function() {
        if(click == 2)
        {
            ...
            console.log(thisnode.obj.animate());
            //thisnode.displayWhenStopMoving();
            return true;
        }
        else if (click == 1)
        {
    ...
    this.el.draggable({
      ...
      start: function() {
        ...
        click=-1; //If we drag, don't exe, onclick function
      },
      drag: function () {
...
Node.prototype.displayWhenStopMoving = function ()
{
  //if i'm not root
  if(this.parent!=null)
  {
    if(this.parent.parent!=null)
    {
      for(var i=0; i<this.parent.children.length; i++)
      {
          if(this.parent.children[i]==this)
          {
            ...
            this.parent.children[i].updatePosition();
            console.log("show "+this.parent.children[i].label());
          }
          else
          {
            this.parent.children[i].hide();
            this.parent.children[i].updatePosition();
            console.log("hide "+this.parent.children[i].label());
          }
      }
  ...
  //create new node
  Node.prototype.createNewNode = function(parentNode, text, onClickFunction, struct)  //fet per mi
  {
...
/* MAP */
  ...
  var Buzzmap = function(el, options) {
    ...
    //NEW Container resize:
    $("#container").on('showhide', function(e) {
      obj.animate();
    });
